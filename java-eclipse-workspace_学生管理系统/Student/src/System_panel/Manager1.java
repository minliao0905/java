...
public class Manager1 extends Box{
	...
	private static final long serialVersionUID = 1L;
	    ...
	    private Vector<Vector> Manager1Data;
	    private DefaultTableModel tableModel;
        Manager_SQl managersql = new Manager_SQl();
        
        
        
	    public Manager1(JFrame jf){
         ...
         //变更选择框
         JLabel jlchange = new JLabel("学籍变更情况：");
         JComboBox<String> changejcb = new JComboBox<String>();
      
         changejcb.addItem("休学1");
         changejcb.addItem("复学2");
         changejcb.addItem("退学3");
         changejcb.addItem("毕业4");
         changejcb.addItem("转系5");
	   ...
	   //学生变更表
	        String[] ts = {"记录号","学籍变更情况","姓名","学号","记录时间","详细描述"}; 
	        titles = new Vector<>();
	        for (String title : ts) {
	            titles.add(title);
	        }
	        Manager1Data = new Vector<>();
	        tableModel = new DefaultTableModel(Manager1Data,titles);   
     ...
     //添加按钮事件
				...
				public void actionPerformed(ActionEvent e) {
	               ...
	               change = change.substring(change.length()-1,change.length());
	            	...
	            	check =	checktext(params);   //检查数据和id
				  	if(check==true) {
				  		 try {
								managersql.InsertIntochange(params);
		       ...
		       updateBtn.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
		            	...
		            	check =	checktext(params);   //检查数据和id
					  	if(check==true) {
					  		 try {
									managersql.updateIntochange(params,id);
								} catch (SQLException e1) {
									// TODO Auto-generated catch block
									e1.printStackTrace();
								}
		   ...
		   //检查数据是否为空，和id重复；
			...
			public void requestData() throws SQLException{
			       ...
			       Manager1Data.clear();
				Vector<Vector> vectors = managersql.getManager1Data();
			      for(Vector vector :vectors) {
			    	  Manager1Data.add(vector);
			      }
